...
//Set the material properties
  gl.uniform4f(vc.umAmb,  this.ambient.X,   this.ambient.Y,  this.ambient.Z,  this.ambient.W);
  gl.uniform4f(vc.umDiff, this.diffuse.X,   this.diffuse.Y,  this.diffuse.Z,  this.diffuse.W);
  gl.uniform1f(vc.umShin, this.shininess.X);
  gl.uniform4f(vc.umSpec, this.specular.X,  this.specular.Y, this.specular.Z, this.specular.W);
...
//Draw the geometry
  ...
  if (selected > -1 && index == selected) {
    gl.uniform4f(vc.umDiff, 1.0, 0.0, 0.0, 0.0);
...
var Light = function() {
  this.ambient = new Coord();
  this.ambient.X = 0.2; this.ambient.Y = 0.2; this.ambient.Z = 0.2;
  this.diffuse = new Coord();
  this.diffuse.X = 1.0; this.diffuse.Y = 1.0; this.diffuse.Z = 1.0;
  this.position = new Coord();
  this.ambient.X = 0.0; this.ambient.Y = 0.0; this.ambient.Z = 2.0; this.position.W = 0.0;
  this.specular = new Coord();
  this.specular.X = 1.0; this.specular.Y = 1.0; this.specular.Z = 1.0;
  this.lightIdx = lights.count++;

//Update the global light dictionary
  lights.amb.push(vec4(this.ambient.X, this.ambient.Y, this.ambient.Z, this.ambient.W));
  lights.diff.push(vec4(this.diffuse.X, this.diffuse.Y, this.diffuse.Z, this.diffuse.W));
  if(lights.pos.length == 0) lights.pos.push(vec4(this.position.X, this.position.Y, this.position.Z, this.position.W));
  lights.spec.push(vec4(this.specular.X, this.specular.Y, this.specular.Z, this.specular.W));
  
  if(lights.count >= 5) {
    $("button.add-light").remove();
  }
};

Light.prototype.render = function(bc, vc) {
  lights.amb[this.lightIdx] = (vec4(this.ambient.X, this.ambient.Y, this.ambient.Z, this.ambient.W));
  lights.diff[this.lightIdx] = (vec4(this.diffuse.X, this.diffuse.Y, this.diffuse.Z, this.diffuse.W));
  lights.pos[0] = (vec4(this.position.X, this.position.Y, this.position.Z, this.position.W));
  lights.spec[this.lightIdx] = (vec4(this.specular.X, this.specular.Y, this.specular.Z, this.specular.W));
  
  gl.uniform4fv(vc.ulAmb,  flatten(lights.amb));
  gl.uniform4fv(vc.ulDiff, flatten(lights.diff));
  gl.uniform4fv(vc.ulPos,  flatten(lights.pos));
  gl.uniform4fv(vc.ulSpec, flatten(lights.spec));
};
...
//Add the object to the list
  ...
  if(t.attr("data-category") == "geometry") {
    ...
    switch(t.attr("data-create")) {
      case "cone":
        geo = ShapeType.CONE;
        break;
        
      case "cylinder":
        geo = ShapeType.CYLINDER;
        break;
        
      case "sphere":
        geo = ShapeType.SPHERE;
        break;
    }
    
    shapes.push(new Renderer(geo, $("input#iterations").spinner("value")));
    associate($("li.list-group-item").length - 1, 0);
  } else {
    shapes.push(new Light());
    associate($("li.list-group-item").length - 1, 7);
  }
...
function bootstrapUI() {
  $("button.add").click(function() {
    ...
    addEntry(this);
    selected = $("li.list-group-item").length - 1;
    
    render();
  });
  
  $("ul.objects").on("click", "li.list-group-item", function() {
    var objIdx = $("li.list-group-item").index(this);
    selected = objIdx;
    
    if(objIdx >= 0) {
      $("li.list-group-item.active").removeClass("active");
      $("li.list-group-item").eq(objIdx).addClass("active");
      
    //Show the controls panel
      var c = $("div.panel");
      c.show();

      if($("li.list-group-item").eq(objIdx).hasClass("geometry")) {
        c.find("button.geometry").removeClass("active").show().first().addClass("active");
        c.find("button.light").hide();
        associate(objIdx, 0);
      } else {
        c.find("button.geometry").hide();
        c.find("button.light").removeClass("active").show().first().addClass("active");
        associate(objIdx, 7);
      }
